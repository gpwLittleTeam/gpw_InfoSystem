...
<script>
	...
	/* function showMembers(groupNo,groupName,obj) {
		...
		TdName = groupNo+'TD';
		$("tr[name="+TdName+"]").slideToggle("quick");
	...
	function drawMember(groupNo,groupName){
		...
		if($("#resultGroup"+groupNo).length==0){
			if(nrOfChoosen=="") {
				alert("输入抽取人数");
				return false;
			}
			...
			$.post("drawingMemeberOfGroup.action", {groupNo : groupNo,numberChoosen:nrOfChoosen}, function(data) {
				...
				if(theErrors == null) {
					var htmlBlock = 
						"<tr class='list name' id=" + "resultGroup" + groupNo + ">" + 
							"<td>"+ groupName +"</td>" + 
							"<td>";
					...
					$.each(leaderResult,function(i,leaderResult){
						htmlBlock += "<a target='_blank' href='to_infoBrowsing.action?expert_no="+ leaderResult.expert_Field1 +"'>" + leaderResult.expert_Field2 + "</a>";
					...
					if(theErrors == "NoLeaderToRandom") {
						alert("该专业组中没有拟任组长的专家");
					} else if(theErrors == "NoMemberToRandom") {
						alert("该专业组中没有拟任组员的专家");
					}
				}
			});
		} else {
		...
		if($("#resultGroup"+groupNo).length==0){
			...
			$.post("drawingMemeberOfGroup.action", {groupNo : groupNo,numberChoosen:nrOfChoosen}, function(data) {
				...
				var htmlBlock = 
					"<tr class='list name' id=" + "resultGroup" + groupNo + ">" + 
						"<td>"+ groupName +"</td>" + 
						"<td><a target='_blank' href='./zjgl_infoBrowsing.jsp'>";
				var leaderResult = JSON.parse(data.leaderResult);
				var memberResult = JSON.parse(data.memberResult);
				//console.log(leaderResult); //
				//console.log(memberResult); //
				$.each(leaderResult,function(i,leaderResult){
					htmlBlock += leaderResult.expert_Field2;
				});
				htmlBlock += "</a></td><td>";
				$.each(memberResult,function(i,memberResult){
				...
				htmlBlock += "</td></tr>";
				$("#resultGroup"+groupNo).replaceWith(htmlBlock);
				}
			});
		}
	}
						...
						<td bgcolor="#f3ffe3" align="center">
								<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" style="margin:0 0 10px 0"
									...
									<tr>
										...
										<th width="22.5%" height="18" background="images/tab_14.gif" class="STYLE1">抽取人数</th>
									...
									<s:iterator id="group" value="listGroup" status="index">
										<tr class="list operation">
											...
											<td><a onclick="showMembers('<s:property value="#group.Group_no"/>','<s:property value="#group.Group_name"/>',this)"><s:property value="#group.Group_name" /></a></td>
										...
										<tr name="<s:property value="#group.Group_no"/>TD"  style="display: none">
