...
//	private String juryName;
	...
	private List<String> idcodeAndTime;
...
//		System.err.println("session User:"+objUserLogin);
		...
		idcodeAndTime = objGetJuryIdcode.getJuryIdcodeByNamePhone(expertName, expertPhone);
			//日期处理
			Date now = new Date();
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

			if(!idcodeAndTime.isEmpty()){
				long timeDistance = now.getTime() - sdf.parse(idcodeAndTime.get(1)).getTime();
				
				//如果姓名号码验证码都对且未超时
				if(idcodeAndTime.get(0).equals(expertIdcode) && (timeDistance/(60*60*1000)) < 24){
					//得到对应的juryNo
...
//					System.out.println("juryName:" + juryName);
				...
				else if(!idcodeAndTime.get(0).equals(expertIdcode)){
					message = "姓名或号码或验证码错误";
					return ERROR;
				}
				else{
...
//			System.out.println(now + " db:" + sdf.parse(psdAndTime.get(1)));
	...
	public List<String> getIdcodeAndTime() {
		return idcodeAndTime;
	}


	public void setIdcodeAndTime(List<String> idcodeAndTime) {
		this.idcodeAndTime = idcodeAndTime;
	}
